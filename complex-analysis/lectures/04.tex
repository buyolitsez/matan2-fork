...
\end{thm}
\begin{proof}[\normalfont\textsc{Доказательство}]
	По условию существует точка $ z_\ast \in \Omega $ такая, что для любого $ \rho > 0 $ $ B(z_\ast,\rho) \setminus \left\{ z_\ast \right\} \cap E \neq \varnothing $. Рассмотрим функцию $ h = f - g $ и докажем, что существует $ r > 0 $ такое, что $ B(z_\ast,r) \subset \Omega $ и $ h|_{B(z_\ast,r)} \equiv 0 $. По лемме \eqref{lemma:zero_multiplicity} о кратности нуля
 \begin{align}
